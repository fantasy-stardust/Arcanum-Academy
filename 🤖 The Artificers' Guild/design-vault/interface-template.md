# Interface Template | 接口模板

Use this template when one hall's work must be handed to another.

当一个分殿的成果需要交接给另一个分殿时，可使用此模板。

---

## Interface Name | 接口名称

`[source]-to-[target]-[purpose]`

## Source Hall | 来源分殿

- Hall: 
- Owned part: 
- Expected output: 

## Target Hall | 目标分殿

- Hall: 
- Dependent part: 
- Expected input: 

## Conditions | 条件

- Normal operating range 正常工作范围
- Failure conditions 失效条件
- Safety notes 安全说明

## Proof Before Handoff | 交接前证明

- What has been tested 已测试内容
- What remains uncertain 尚未确定内容
- Who approved the handoff 交接批准者

## Revision Record | 修订记录

- Revision mark 修订标记
- Date 日期
- Reason 理由

## Closing Question | 结尾核问

Can the receiving hall understand and use this without guessing?

接收分殿是否可以在不靠猜测的情况下理解并使用它？